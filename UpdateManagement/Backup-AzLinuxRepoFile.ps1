...
$null = Get-AzStorageBlobContent -Blob "linuxbackuprepofiles.sh" -Container "invokecommands" -Context $saContext -Destination "C:\Temp"
#endregion Download shell script

#region Invoke shell script on virtual machine
Write-Output "INFORMATION - Process to backup repository files in progress"
foreach ($linuxItem in $linuxQueryResult) {
	...
	$invokeResult = Invoke-AzVMRunCommand -ResourceGroupName $linuxObject.RgName -Name $linuxObject.VmName -CommandId 'RunShellScript' -ScriptPath "C:\Temp\linuxbackuprepofiles.sh"
...
#region Invoke shell script on virtual machine
Write-Output "INFORMATION - Process to backup repository files has completed"