...
$dbMandatoryTables = @("activedirectory_patchwave", "subscription_permission_check") # Here should be tables that are needed to get information for schedule
$dbTableCounter = 0
...
$dbGetQuery = "
    SELECT pwname, pwday, pwstart, pwduration, pwisutc, pwregion FROM dbo.activedirectory_patchwave WHERE pwregion = '$pwRegionInDb' AND pwisused = 1;
    "
$dbConParams.Query = $dbGetQuery
...
$timeZoneList = @{} # Timezone list for Active Direcotry servers is empty because AD servers are always in UTC time
...
$dbConParams.Query = ""
$regexCheckPwM = [System.Text.RegularExpressions.Regex]::new("GRP_ADS_pw\d[M]_g\d", "ignoreCase")
# Variables end
...
foreach ($patchGroupObject in $patchGroupObjectList) {
    ...
    # After change freeze, patching starts always with PW1A.
    ...
    foreach ($scheduleStartTime in $scheduleStartTimeArray) {
        ...
        $scheduleName = "ActiveDirectory-$($patchGroupObject.PwName.Remove(0,4).Replace('_','-'))-$scheduleDateToString"
    ...
    foreach ($updateSchedule in $updateScheduleArray) {
        if ($updateSchedule.StartTime.UtcDateTime -gt $currentDateUtc) {
            $pwtUpdateConfigParams = @{
                ...
                RebootSetting =
                if ($regexCheckPwM.IsMatch($patchGroupObject.PwName)) {
                    "Never"
                }
                else {
                    "Always"
...
foreach ($pathcObject in $patchGroupObjectList) {
    ...
    $patchObjectName = "ActiveDirectory-" + $patchObjectName
    ...
    if ($scheduleUpdateSuccessful) {
        ...
        $dbUpdateQuery = "
            UPDATE dbo.activedirectory_patchwave SET pwisplanned = 1 WHERE pwname = '$($pathcObject.PwName)' AND pwregion = '$pwRegionInDb';
            "
